...
//Get all ispindles
$q_sql_distinctName = mysql_query("SELECT DISTINCT Name FROM Data ORDER BY Name DESC LIMIT 5;") or die(mysql_error());
	//$r_distinctNameArray = mysql_fetch_array($q_sql_distinctName);

	$i = 0;
    while($r_distinctName = mysql_fetch_array($q_sql_distinctName))
    {
		$Name_ii = "MyIspindle".$i++;
	// Set session variables
      $_SESSION[$Name_ii] = $r_distinctName['Name'];
	  //echo $Name_ii;
    }	


$iSpindleID = $_SESSION["MyIspindle0"];
...
function getChartValues($iSpindleID, $timeFrameHours=24)
{
...
function getChartValuesPlato($iSpindleID, $timeFrameHours=24)
{
...
function getCurrentValues($iSpindleID)
{
...
function getChartValuesPlato4($iSpindleID, $timeFrameHours=24)
{
